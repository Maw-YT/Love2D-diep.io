...
function Game:load()
    ...
    self.ArenaFactory = self.res.ArenaFactory
    self.arenas = {}
    self.gamemode = self.gamemode or "FFA"
    self.arena = self:createArenaForGamemode(self.gamemode, nil)
    ...
    self.notificationSystem = self.res.NotificationSystem:new()
end

function Game:createArenaForGamemode(gamemode, player)
    local arena = self.ArenaFactory.create(gamemode, 22300, 22300, player)
    arena.game = self
    table.insert(self.arenas, arena)
    return arena
end

function Game:discardArenasWithoutPlayers()
    for i = #self.arenas, 1, -1 do
        local arena = self.arenas[i]
        if arena.player == nil then
            if arena.dispose then
                arena:dispose()
            end
            table.remove(self.arenas, i)
        end
    end
end
...
function Game:startGame()
    ...
    -- Discard old arenas that have no player attached.
    if self.arena then
        self.arena.player = nil
    end
    self:discardArenasWithoutPlayers()

    -- Create a fresh arena for selected gamemode.
    self.arena = self:createArenaForGamemode(self.gamemode, nil)
    
    -- 2. Initialize the Arena with baseline shapes
    self.arena:spawnInitialShapes(4000)
    self.arena.bosses = {}
    self.arena.bossSpawnTimer = 0
    ...
    elseif self.gamemode == "TwoTeams" then
        self.player:setTeam("blue")
        for i = 1, 12 do
            if i % 2 == 0 then
                self.arena:spawnBot("blue")
            else
                self.arena:spawnBot("red")
            end
        end
    else
...
function Game:update(dt)
    ...
    elseif self.state == "PLAYING" then
        ...
        self.arena:updateBosses(dt, self.camera)
        for _, bot in ipairs(self.arena.bots) do
            for _, b in ipairs(bot.bullets) do b:update(dt, self.arena, self.camera) end
        end
        for _, boss in ipairs(self.arena.bosses) do
            for _, b in ipairs(boss.bullets) do b:update(dt, self.arena, self.camera) end
        end
        self.arena:updateShapes(dt, self.camera)
...
function Game:respawnPlayer()
    ...
    local margin = 100
    local spawnX
    if self.gamemode == "TwoTeams" then
        spawnX = love.math.random(margin, math.floor(self.arena.width * 0.20))
    else
        spawnX = love.math.random(margin, self.arena.width - margin)
    end
    local spawnY = love.math.random(margin, self.arena.height - margin)
    -- Create a new player instance at the random location
    if self.player then
        self.arena.entityManager:remove(self.player)
    end
    self.player = nil
    self.arena.player = nil
    ...
    self.arena.entityManager:add(self.player)
...
function Game:draw()  
    ...
    if self.state == "PLAYING" or self.state == "PAUSED" then  
        ...
        self.arena:drawBosses(1, self.style, self.camera)
        for _, bot in ipairs(self.arena.bots) do
    ...
    -- Draw notifications (in screen space, not world space)
    if self.notificationSystem then
        self.notificationSystem:draw()
    end
end
