-- game/entities/shape.lua
local EntityBase = require "game.native.entityBase"
local Shape = setmetatable({}, { __index = EntityBase })
Shape.__index = Shape
...
local function chooseTypeFromArena(x, y, arena)
    if arena and arena.gamemode == "TwoTeams" then
...
function Shape:newFromSpawnData(spawnData, arena)
    ...
    self.x = spawnData.x
    self.y = spawnData.y
    self.isCrasher = spawnData.isCrasher or false
    ...
    self:setType(spawnData.type or chooseTypeFromArena(self.x, self.y, arena))
...
function Shape:newRandom(arena)
    local x, y
    if arena and arena.gamemode == "TwoTeams" then
        x = love.math.random(math.floor(arena.width * 0.20), math.floor(arena.width * 0.80))
        y = love.math.random(100, arena.height - 100)
    else
        x = love.math.random(100, arena.width - 100)
        y = love.math.random(100, arena.height - 100)
    end

    return self:newFromSpawnData({
        x = x,
        y = y,
        type = chooseTypeFromArena(x, y, arena),
        isCrasher = false,
    }, arena)
end
