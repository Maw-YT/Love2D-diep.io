...
function Barrel:new(player, fire_delay, bulletType, config)
...
function Barrel:fire()
    ...
    local currentYOffset = self.player.radius * self.yOffsetMult
    local currentLength = self.player.radius * self.lengthMult
    local currentXOffset = -(self.xOffsetMult * self.player.radius)
    
    local combinedAngle = self.player.angle + self.angleOffset
    local spawnDist = self.player.radius + currentLength - currentYOffset
    
    local bx = self.player.x + math.cos(combinedAngle) * spawnDist + math.sin(combinedAngle) * currentXOffset
    local by = self.player.y + math.sin(combinedAngle) * spawnDist - math.cos(combinedAngle) * currentXOffset
