...
local Player = setmetatable({}, { __index = EntityBase })
...
function Player:update(dt, arena, cam)
    ...
    if love.keyboard.isDown("e") and not self.e_pressed then
        ...
        if arena and arena.game and arena.game.notificationSystem then
            arena.game.notificationSystem:notifyAutoFire(self.autoFire)
        end
    end
    ...
    if love.keyboard.isDown("c") and not self.c_pressed then
        ...
        if arena and arena.game and arena.game.notificationSystem then
            arena.game.notificationSystem:notifyAutoSpin(self.autoSpin)
        end
    end
    ...
    while #self.drones > maxAllowed do
        ...
        if oldestDrone.setDead then
            oldestDrone:setDead(true)
        else
            oldestDrone.isDead = true
            oldestDrone.isdead = true -- Legacy compatibility.
        end
    end
    self.healthBar:update(dt, self.health, self.max_health)
end

-- Handle Necromancer shape conversion
function Player:convertShapeToDrone(shape)
    if self.tankName ~= "Necromancer" then return end
    if shape.sides ~= 4 then return end -- Convert only squares
    
    -- Create new drone from converted shape
    local angle = math.random() * math.pi * 2
    local speed = 250
    local vx = math.cos(angle) * speed
    local vy = math.sin(angle) * speed
    
    local newDrone = self.res.Drone:new(self, shape.x, shape.y, vx, vy)
    newDrone.droneType = "square" -- Necromancer drones are always squares
    newDrone.radius = shape:getRadius() or self.radius * 0.7 -- Size based on original shape
    newDrone.damage = shape.max_health -- Damage based on shape's health
    newDrone.penetration = 1
    newDrone.lifetime = 60.0

    -- Only add to bullets list, NOT drones list
    -- This bypasses the maxDrones limit - converted squares can accumulate infinitely
    table.insert(self.bullets, newDrone)
    return newDrone
end
