...
local Bot = setmetatable({}, { __index = EntityBase })
Bot.__index = Bot
...
local function isEnemy(self, target)
    if not target then return false end
    if self == target then return false end
    if self.team == nil or target.team == nil then return true end
...
local function pickTarget(self, arena)
    ...
    if player and not player.isDead and isEnemy(self, player) then
    ...
    for _, other in ipairs(arena.bots) do
        if other ~= self and other.health > 0 and isEnemy(self, other) then
...
function Bot:update(dt, arena, cam)
    ...
    while #self.drones > maxAllowed do
        ...
        if oldest.setDead then
            ...
            oldest.isDead = true
            oldest.isdead = true -- Legacy compatibility.
        end
    end
