...
local ClassWheel = require "game.ui.class_wheel"
...
function UIManager:new(game)
    ...
    -- Class wheel state
    self.showClassWheel = false
    self.classWheelAnimationTimer = 0
    ...
    self.menus = {
        ...
        PAUSED = {
            returnMenuButton = game.res.Button:new("RETURN TO MENU", 0, 0, 240, 50, function()
                local player = game.player
                if player and game.arena and game.arena.entityManager then
                    game.arena.entityManager:remove(player)
                end
                if player then
                    player.bullets = {}
                    player.drones = {}
                end
                if game.dyingObjects and player then
                    for i = #game.dyingObjects, 1, -1 do
                        if game.dyingObjects[i] == player then
                            table.remove(game.dyingObjects, i)
                        end
                    end
                end
                if game.arena then
                    game.arena.player = nil
                end
                game.player = nil
                game.respawnTimer = 5
                if game.discardArenasWithoutPlayers then
                    game:discardArenasWithoutPlayers()
                end
                game.state = "MENU"
            end),
        }
...
function UIManager:update(dt, state)
    ...
    if self.menus[state] then
        for name, element in pairs(self.menus[state]) do
            ...
            elseif name == "returnMenuButton" then
                element.x, element.y = screenW / 2 - 120, screenH / 2 + 70
            elseif name == "FFABtn" then
                local gap = 14
                local mBtnW = 120
                local modeStartX = (screenW / 2) - (((mBtnW * 3) + (gap * 2)) / 2)
                element.x, element.y = modeStartX, screenH / 2 - 250
            elseif name == "SandboxBtn" then
                local gap = 14
                local mBtnW = 120
                local modeStartX = (screenW / 2) - (((mBtnW * 3) + (gap * 2)) / 2)
                element.x, element.y = modeStartX + mBtnW + gap, screenH / 2 - 250
            elseif name == "TwoTeamsBtn" then
                local gap = 14
                local mBtnW = 120
                local modeStartX = (screenW / 2) - (((mBtnW * 3) + (gap * 2)) / 2)
                element.x, element.y = modeStartX + ((mBtnW + gap) * 2), screenH / 2 - 250
            end
            element:update(dt)
    ...
    if self.game.player then
        ...
        -- Handle TAB key for class wheel toggle
        if love.keyboard.isDown("tab") and not self.tabPressed then
            self.showClassWheel = not self.showClassWheel
        end
        self.tabPressed = love.keyboard.isDown("tab")
        
        -- Update class wheel animation
        if self.showClassWheel then
            self.classWheelAnimationTimer = math.min(1, self.classWheelAnimationTimer + dt * 5)
        else
            self.classWheelAnimationTimer = math.max(0, self.classWheelAnimationTimer - dt * 5)
        end
    end
    ...
    if state == "OPTIONS" then
        ...
        -- 6. Check Class Wheel hover (visual only, no clicking)
    if self.showClassWheel and self.classWheelAnimationTimer > 0.5 and self.game.player then
        -- Simple hover detection - just check if mouse is near center area
        local screenW, screenH = love.graphics.getDimensions()
        local centerX, centerY = screenW/2, screenH/2 + 20
        local dx, dy = mx - centerX, my - centerY
        local distFromCenter = math.sqrt(dx*dx + dy*dy)
        
        -- Hover if within the wheel area (max radius ~350)
...
function UIManager:draw(state, player)
    ...
    elseif state == "PAUSED" then
        ...
        for _, btn in pairs(self.menus.PAUSED) do btn:draw() end
        ...
        -- Draw class wheel if active or animating
        if self.classWheelAnimationTimer > 0 then
            ClassWheel.draw(player, self.classWheelAnimationTimer)
        end
...
function UIManager:mousepressed(x, y, button)
    ...
    if self.game.state == "PLAYING" then
        ...
        -- Note: Class Wheel is non-clickable (visual reference only)
    -- Upgrades are handled through the regular upgrade menu
end
