...
local function bulletHitTank(bullet, tank, dt, game)
    ...
    if tank.health <= 0 and not tank.isDead and game then
        ...
        -- Notifications (only when player is involved)
        if game.notificationSystem then
            -- Player killed a bot
            if bullet.player == game.player and tank.name then
                game.notificationSystem:notifyKilled(tank.name)
            -- Player was killed by a bot
            elseif tank == game.player and bullet.player and bullet.player.name then
                game.notificationSystem:notifyKilledBy(bullet.player.name)
            end
        end
    end
    ...
    for _, bot in ipairs(bots) do
        ...
        if distSq < min * min then
            ...
            if player.health <= 0 and not player.isDead then
                ...
                -- Notification: Player was killed by bot (body damage)
                if game.notificationSystem and bot.name then
                    game.notificationSystem:notifyKilledBy(bot.name)
                end
            end
            if bot.health <= 0 and not bot.isDead then
                ...
                -- Notification: Player killed bot (body damage)
                if game.notificationSystem and bot.name then
                    game.notificationSystem:notifyKilled(bot.name)
                end
            end
        end
        ::nextbot::
